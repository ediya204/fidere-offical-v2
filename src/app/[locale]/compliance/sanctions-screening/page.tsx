...
export async function generateMetadata({ params }: Props) {
  ...
  return pageMetadata(locale, "compliance/sanctions-screening", text(locale, ...topic.title), text(locale, ...topic.description));
}

export default async function Page({ params }: Props) {
  const { locale } = await params;
  if (!isLocale(locale)) notFound();
  return <ComplianceTopic locale={locale} slug="sanctions-screening" />;
}
