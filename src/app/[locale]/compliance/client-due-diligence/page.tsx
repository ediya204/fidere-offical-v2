...
export async function generateMetadata({ params }: Props) {
  ...
  return pageMetadata(locale, "compliance/client-due-diligence", text(locale, ...topic.title), text(locale, ...topic.description));
}

export default async function Page({ params }: Props) {
  const { locale } = await params;
  if (!isLocale(locale)) notFound();
  return <ComplianceTopic locale={locale} slug="client-due-diligence" />;
}
