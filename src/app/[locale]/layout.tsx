...
export default async function LocaleLayout({children,params}:{children:React.ReactNode;params:Promise<{locale:string}>}) {
 ...
 return <html lang={localeSeo[locale].htmlLang} dir={locale==="ar"?"rtl":"ltr"} className={`${editorial.variable} ${body.variable}${locale==="ar"?` ${arabicEditorial.variable} ${arabicBody.variable}`:""}`}><body id="top"><a className="skip-link" href="#main-content">{text(locale,"Skip to content","跳至主要內容","跳至主要内容")}</a><Header locale={locale}/><main id="main-content">{children}</main><Footer locale={locale}/><MotionEnhancements/><Script src={`https://www.googletagmanager.com/gtag/js?id=${googleAnalyticsId}`} strategy="afterInteractive"/><Script id="google-analytics" strategy="afterInteractive">{`window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag('js',new Date());gtag('config','${googleAnalyticsId}');`}</Script></body></html>;
}
