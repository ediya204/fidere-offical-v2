...
export async function generateMetadata({ params }: Props) {
  ...
  return pageMetadata(locale, "wealth-management/funds", text(locale, ...topic.title), text(locale, ...topic.description));
}

export default async function Page({ params }: Props) {
  const { locale } = await params;
  if (!isLocale(locale)) notFound();
  return <WealthTopicPage locale={locale} slug="funds" />;
}
