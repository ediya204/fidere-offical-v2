...
2026-09-09，生产页面：[https://www.fideretrust.com/ar](https://www.fideretrust.com/ar)；本地预览：<http://127.0.0.1:3000/ar>。
...
- 750 条当前共享文案翻译，另有动态导航/表单与图片说明的显式阿拉伯语文案；无中文或英文静默回退。
...
- 当前三语构建结果：`node scripts/validate-arabic.mjs --offline` 验证 750 条阿拉伯语词条、117个预渲染页面，其中 Arabic 39 页；sitemap 为 114 个 URL，每语言 38（排除登录页）。校验同时拒绝任何残留的 `/zh-hans` 预渲染页面或 hreflang/sitemap 入口。报告：`artifacts/arabic-i18n/validation.json`，检查时间13:23:57 HKT，Build ID 为 `Qzojjab6lVt_-pMP7_zZc`。
...
未知地址现由 `npm run cf:build` 在静态导出后生成统一品牌 404。页面按首段路径显示英文、繁中或阿拉伯语，保留真实 HTTP 404 与 `noindex, nofollow`，并在五秒后返回对应语言首页；用户可立即返回或取消。项目仍未启用实验性 `global-not-found`，因为它曾在本项目的多根布局下对不存在的 Insights slug 触发 NoFallbackError / 500。此项变更已在本地 Wrangler 及生产浏览器验证，并随部署 `74f310b3-cb55-41e2-9746-6920e4588c7b` 发布。
...
当前三语实现已部署到 `https://www.fideretrust.com`；阿拉伯语首页、详情页、RTL、导航与品牌 404 已完成生产抽查。本次发布没有修改 DNS 或自定义域名配置。
